...
public class AddMemberActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private void saveMember() {
        ...
        if(TextUtils.isEmpty(firstName)) {
            Toast.makeText(this, "Can't save - Insert a firstname!", Toast.LENGTH_LONG).show();
            return;
        } else if(TextUtils.isEmpty(lastName)) {
            Toast.makeText(this, "Can't save - Insert a lastname!", Toast.LENGTH_LONG).show();
            return;
        } else if(TextUtils.isEmpty(sport)) {
            Toast.makeText(this, "Can't save - Insert a sport!", Toast.LENGTH_LONG).show();
            return;
        } else if(gender == MemberEntry.GENDER_UNKNOWN) {
            Toast.makeText(this, "Can't save - Insert a gender!", Toast.LENGTH_LONG).show();
            return;
        }
        else {
            ...
            if (currentMemberUri == null) {
                ...
                if (uri == null) {
                    ...
                    Toast.makeText(this, "Insertion is done!", Toast.LENGTH_LONG).show();
                }
            } else {
                int rowsChanged = getContentResolver().update(currentMemberUri, contentValues, null, null);
                if (rowsChanged == 0) {
                    Toast.makeText(this, "Saving is failed", Toast.LENGTH_LONG).show();
                } else {
                    Toast.makeText(this, "Member was updated!", Toast.LENGTH_LONG).show();
                }
            }
        }
    ...
    public void onLoadFinished(@NonNull Loader<Cursor> loader, Cursor data) {
        if (data.moveToFirst()) {
            ...
            int sportColumnIndex = data.getColumnIndex(MemberEntry.KEY_SPORT);

            firstNameEditText.setText(data.getString(firstNameColumnIndex));
            lastNameEditText.setText(data.getString(lastNameColumnIndex));
            sportEditText.setText(data.getString(sportColumnIndex));

            int genderInt = data.getInt(genderColumnIndex);
    ...
    private void deleteMember() {
        if (currentMemberUri != null) {
            ...
            if (rowsDeleted == 0) {
                Toast.makeText(this, "No row was deleted!", Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(this, "Member was deleted!", Toast.LENGTH_LONG).show();
            }
            finish();
        }
    }
